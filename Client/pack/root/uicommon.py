...
class PopupDialog(ui.ScriptWindow):
	...
	def SetText(self, text):
		...
		if text and self.message.GetTextLineCount() > 1:
			(x, y) = self.message.GetLocalPosition()

			self.message.SetPosition(x, 38 - (self.message.GetTextLineCount() - 1) * 12)
...
class QuestionDialog(ui.ScriptWindow):
	...
	def SetText(self, text):
		...
		if text and self.textLine.GetTextLineCount() > 1:
			(x, y) = self.textLine.GetLocalPosition()
			self.textLine.SetPosition(x, 38 - (self.textLine.GetTextLineCount() - 1) * 10)
