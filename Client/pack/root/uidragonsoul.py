...
class DragonSoulWindow(ui.ScriptWindow):
	...
	if app.ENABLE_DS_GRADE_MYTH:
		PAGE_TAB_TITLES = [uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_1, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_2,
			uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_3, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_4, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_5, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_6]
	...
	if app.__BL_MULTI_LANGUAGE__:
		...
		def ReloadVariables():
			...
			DragonSoulWindow.PAGE_TAB_TITLES =(
				uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_1, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_2,
				uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_3, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_4,
				uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_5, uiScriptLocale.DRAGONSOUL_PAGE_BUTTON_6,
			)
	...
	def __LoadWindow(self):
		...
		try:
			...
			if app.ENABLE_DS_GRADE_MYTH:
				if localeInfo.IsARABIC():
					pyScrLoader.LoadScriptFile(self, "UIScript/dragonsoulwindowae.py")
				else:
		...
		try:
			if localeInfo.IsARABIC():
				board = self.GetChild("Equipment_Base")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_01")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_02")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_03")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_04")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_05")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
				board = self.GetChild("Tab_06")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
			...
			self.inventoryTab = []
			for i in range(1, player.DRAGON_SOUL_PAGE_COUNT+1):
		...
		self.deckTab[0].Down()
		## Grade button
		for i in xrange(player.DRAGON_SOUL_PAGE_COUNT):
	...
	def SetInventoryPage(self, page):
		...
		for i in range(1, player.DRAGON_SOUL_PAGE_COUNT):
			self.inventoryTab[(page+i) % player.DRAGON_SOUL_PAGE_COUNT].SetUp()
		self.RefreshBagSlotWindow()
...
class DragonSoulRefineWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			if localeInfo.IsARABIC():
				board = self.GetChild("DragonSoulRefineWindowBaseImage")
				board.SetScale(-1.0, 1.0)
				board.SetRenderingRect(-1.0, 0.0, 1.0, 0.0)
