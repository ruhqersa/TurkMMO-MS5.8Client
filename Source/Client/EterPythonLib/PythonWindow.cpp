...
namespace UI
{
	...
	void CTextLine::SetLineHeight(int lineHeight)
	{
		m_TextInstance.SetLineHeight(lineHeight);
	}
	...
	void CAniImageBox::AppendImage(const char * c_szFileName)
	{
		...
		pImageInstance->SetScale(m_v2Scale.x, m_v2Scale.y);
		m_ImageVector.push_back(pImageInstance);
		m_bycurIndex = static_cast<unsigned char>(rand() % m_ImageVector.size());
	}
	...
	struct FSetAniImgScale
	{
		...
		void operator () (CGraphicExpandedImageInstance* pInstance)
		{
			pInstance->SetScale(v2Scale.x, v2Scale.y);
		}
	};
	...
	void CAniImageBox::SetAniImgScale(float fScaleX, float fScaleY)
	{
		...
		setAniImgScale.v2Scale.x = fScaleX;
		setAniImgScale.v2Scale.y = fScaleY;
