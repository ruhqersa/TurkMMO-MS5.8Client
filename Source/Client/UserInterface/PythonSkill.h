...
class CPythonSkill : public CSingleton<CPythonSkill>
{
	...
	public:
		...
		enum EPetSkillToken
		{
			PET_SKILL_TOKEN_INDEX,
			PET_SKILL_TOKEN_NAME,
			PET_SKILL_TOKEN_ICON_NAME,
			PET_SKILL_TOKEN_TYPE,
			PET_SKILL_TOKEN_DESCRIPTION,
			PET_SKILL_TOKEN_COOLDOWN,
		...
		typedef std::map<int, TPetSkillData> TPetSkillDataMap;

		bool	RegisterPetSkillTable(const char* c_szFileName);
		bool	GetPetSkillData(DWORD dwSkillIndex, TPetSkillData** ppPetSkillData);
	protected:
