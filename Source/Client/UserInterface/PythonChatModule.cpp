...
PyObject * chatGetLinkFromHyperlink(PyObject * poSelf, PyObject * poArgs)
{
	...
	else if (0 == results[0].compare("petitem"))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData))
		{
			...
			if (pPet)
			{
				len += snprintf(itemlink + len, sizeof(itemlink) - len, "%x:%d:%x:%x:%x:%x:%d:%x:%x:%x:%s:%x:%x:%x:%x",
					htoi(results[6].c_str()),
					static_cast<int>(atoi(results[7].c_str())), // %d espera int, atoi devuelve int
					htoi(results[8].c_str()),
					htoi(results[9].c_str()),
					htoi(results[10].c_str()),
					htoi(results[11].c_str()),
					static_cast<int>(atoi(results[12].c_str())), // %d espera int, atoi devuelve int
					htoi(results[13].c_str()),
					htoi(results[14].c_str()),
					htoi(results[15].c_str()),
					results[16].c_str(), // %s espera const char*, c_str() devuelve const char*
					htoi(results[17].c_str()),
					htoi(results[18].c_str()),
					htoi(results[19].c_str()),
					htoi(results[20].c_str())
				);
			}
