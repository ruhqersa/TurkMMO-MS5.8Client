...
void CGrannyLODController::UpdateLODLevel(float fDistanceFromCenter, float fDistanceFromCamera)
{
	...
	if (fDistanceFromCenter > LOD_APPLY_MIN)
	{
		...
		if (m_fLODDistance>0.0f)
			m_dwLODAniFPS = static_cast<DWORD>((static_cast<float>(CGrannyModelInstance::ANIFPS_MAX) - static_cast<float>(CGrannyModelInstance::ANIFPS_MIN)) * fLODFactor / m_fLODDistance + static_cast<float>(CGrannyModelInstance::ANIFPS_MIN));
		else
